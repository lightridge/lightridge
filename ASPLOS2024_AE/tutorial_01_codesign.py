...
class DiffractiveClassifier_Codesign(torch.nn.Module):
    ...
    def prop_view(self, x):
        ...
        for i in range(x.shape[0]):
            print(i)
            utils.forward_func_visualization(prop_list, self.size, fname="codesign_mnist_%s.pdf" % i, idx=i, intensity_plot=False)
        output = self.detector(x)
        return
    ...
    def phase_view(self, cmap="hsv"):
        ...
        utils.phase_visualization(phase_list,size=self.size, cmap=cmap, fname="phase_view_codesign.pdf")
        return
...
def main(args):
    ...
    model = DiffractiveClassifier_Codesign(num_layers=args.depth, batch_norm =args.use_batch_norm,device=device, 
			#det_y_loc = [105, 125, 145, 95, 115, 135, 155, 105, 125, 145], #det_y_loc = [175,195,215,165,185,205,225,175,195,215],
                        ...
                        distance=args.distance, phase_func=phase_function, intensity_func=intensity_function, precision = args.precision,
                        amp_factor=args.amp_factor, approx=args.approx)
    model.to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model-save-path', type=str, default="./save_model_codesign/")
    parser.add_argument('--result-record-path', type=pathlib.Path, default="./result_codesign.csv", help="save training result.")
